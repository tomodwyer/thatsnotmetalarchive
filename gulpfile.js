...
function buildJekyll(incremental) {
  ...
  let cmd = `bundle exec jekyll build --source ${paths.jekyll
    .src} --destination ${paths.jekyll.dist}`;
...
gulp.task("stylesheets", () => {
  ...
  return gulp
    ...
    .pipe(
      postcss([
        autoprefixer({
          browsers: ["> 1% in gb", "ie >= 8", "last 2 versions"]
        })
      ])
    )
...
gulp.task("build", () => {
  runSequence("clean", "javascripts", "stylesheets", "jekyll");
});

gulp.task("default", () => {
  runSequence("clean", "javascripts", "stylesheets", "jekyll", "browserSync", [
    "watchJekyll"
  ]);
});
